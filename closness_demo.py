...
if color == True:
    for i in range(0, len(x_lims) - 1):
        ax.fill_between([x_lims[i], x_lims[i + 1]], np.max(y[x_lims[i]-frame[0]:x_lims[i + 1]-frame[0]+1]) + 0.002, np.min(y[x_lims[i]-frame[0]:x_lims[i + 1]-frame[0]+1])- 0.002, facecolor=cmaps[i], alpha=0.4, interpolate=True)
plt.grid(True)
plt.xlabel('Time (Frame Number)', fontsize=FontSize)
plt.ylabel(centrality_label, fontsize=FontSize)
ax.legend(loc='lower left', bbox_to_anchor=(0, 1.01), ncol=2, borderaxespad=0, fontsize=FontSize - 3, frameon=False)
plt.show()
